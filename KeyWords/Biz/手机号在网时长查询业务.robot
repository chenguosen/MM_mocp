...
读取手机号在网时长查询模板
    [Arguments]    @{kvs}
    Set Test Variable    ${req_flag}    ${TNC_interface_names[3]}
    ...
    ${sign}    Set Variable    ${appkey}clientIp=192.168.16.1clientState=${clientState}expandParams=key:1msisdn=${tel}operatorType=2systemTime=${systemtime}${appkey}
...
扣费记录数据库入库检查
    ${saleprice}    数据库查询_返回单值    SELECT salePrice FROM t_product WHERE productID \ in (SELECT productID FROM t_account4product where accountID in (select accountID FROM T_PreLicense where preLicenseID in (SELECT preLicenseID FROM T_PreLicense4App WHERE appid='${appid}'))) and capabilityType='6';    #获取销售品的客户售价
    ${basisPrice}    数据库查询_返回单值    SELECT basisPrice FROM t_product WHERE productID \ in (SELECT productID FROM t_account4product where accountID in (select accountID FROM T_PreLicense where preLicenseID in (SELECT preLicenseID FROM T_PreLicense4App WHERE appid='${appid}'))) and capabilityType='6';    #获取销售品的运营商购买价
    数据库查询_返回单值    select count(*) from t_onlinetimelog t where t.clientState='${clientState}' and t.isVerify='0' and t.fee='${saleprice}' and t.prefee='${basisPrice}';
