...
*** Test Cases ***
用户未开户，充值数据正常，充值失败
    新用户正常开户    0    $.busiInfo.operType=(int)2
...
用户已开户，充值数据正常，充值成功
    ...
    发送Reqx请求    ${req_flag}
    校验上链交易流水    1    tid='${tid}'    oper_type=2    user_id='${uid}'    cb_flag=1
    校验应答返回码信息    0000
    Sleep    ${G_Chain_Delay}
    校验链上用户余额    ${uid}    ${credit}
    获取回调请求数据    ${ctrlUri}
...
用户已开户，operTime值大于当前时间，记录该时间，充值成功
...
用户已开户，积分值超大，但不超过20位，充值成功
    ...
    发送回调应答包请求
    ${credit1}    Evaluate    random.randint(56,150)    random
    ${credit2}    Evaluate    888888888888888888888
    ${credit}    Evaluate    ${credit1}+${credit2}
    新用户正常开户    ${G_Chain_Delay}    $.busiInfo.operType=(int)1    $.busiInfo.params.operTime=${tran_time}    $.busiInfo.params.credit=${credit1}
    校验链上用户余额    ${uid}    ${credit1}
    ${tran_time}    生成指定时间串
