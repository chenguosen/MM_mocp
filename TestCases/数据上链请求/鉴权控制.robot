...
*** Test Cases ***
IP在缓存中无，数据库中有且允许，数据上链请求成功且加入缓存
...
IP在缓存中无，数据库中有且禁止，数据上链请求失败且加入缓存
    ${localhost}    get host ip
