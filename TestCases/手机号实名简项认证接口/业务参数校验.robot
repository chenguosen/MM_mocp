...
expandParams节点不存在，校验失败，返回错误码
    ...
    TNC发送Reqx请求    /tnc/client/threeFuse/${req_flag}
    校验应答字段值    $.msgResp.msgBody.result=200006    $.msgResp.msgBody.message=请求签名验证失败
...
userName节点不存在，校验失败，返回错误码
    读取手机号码实名简项认证模板
    删除请求节点    $.msgReq.msgBody.userName
    TNC发送Reqx请求    /tnc/client/threeFuse/${req_flag}
    校验应答字段值    $.msgResp.msgBody.result=200006    $.msgResp.msgBody.message=请求签名验证失败

userName值为空串，校验失败，返回错误码
    读取手机号码实名简项认证模板
    更新节点数据    $.msgReq.msgBody.userName=${EMPTY}
    TNC发送Reqx请求    /tnc/client/threeFuse/${req_flag}
    校验应答字段值    $.msgResp.msgBody.result=200006    $.msgResp.msgBody.message=请求签名验证失败

userName值为空，校验失败，返回错误码
    读取手机号码实名简项认证模板
    更新节点数据    $.msgReq.msgBody.userName=${None}
    TNC发送Reqx请求    /tnc/client/threeFuse/${req_flag}
    校验应答字段值    $.msgResp.msgBody.result=200006    $.msgResp.msgBody.message=请求签名验证失败

idCode节点不存在，校验失败，返回错误码
    读取手机号码实名简项认证模板
    删除请求节点    $.msgReq.msgBody.idCode
    TNC发送Reqx请求    /tnc/client/threeFuse/${req_flag}
    校验应答字段值    $.msgResp.msgBody.result=200016    $.msgResp.msgBody.message=身份证号码验证失败

idCode值为空串，校验失败，返回错误码
    读取手机号码实名简项认证模板
    更新节点数据    $.msgReq.msgBody.idCode=${EMPTY}
    TNC发送Reqx请求    /tnc/client/threeFuse/${req_flag}
    校验应答字段值    $.msgResp.msgBody.result=200006    $.msgResp.msgBody.message=请求签名验证失败

idCode值为空，校验失败，返回错误码
    读取手机号码实名简项认证模板
    更新节点数据    $.msgReq.msgBody.idCode=${None}
    TNC发送Reqx请求    /tnc/client/threeFuse/${req_flag}
    校验应答字段值    $.msgResp.msgBody.result=200016    $.msgResp.msgBody.message=身份证号码验证失败
