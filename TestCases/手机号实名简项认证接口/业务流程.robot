...
redis里key不存在，返回错误码
    ...
    校验应答字段值    $.msgResp.msgBody.message=该预授权ID错误    $.msgResp.msgBody.result=300203
...
预授权余额=阀值+单价+0.1厘，扣费成功
    ...
    删除key    KEY_PRELICENSE_BALANCE_${preLicenseID}
    ${saleprice}    获取销售品的客户售单价    ${appid}    3    #3代表能力来类型为手机实名简项认证
...
appid不存在，校验失败，返回错误码
    ...
    TNC发送Reqx请求    /tnc/client/threeFuse/${req_flag}
    校验应答字段值    $.msgResp.msgBody.result=300101    $.msgResp.msgBody.message=该应用不存在
